...
public final class LowMafRefGTRec implements RefGTRec {
    ...
    public long estBytes() {
        int overhead = (1 + (hapIndices.length - 1))*12; // assume 12 bytes overhead per "owned" object
        long estBytes = overhead + (3 + hapIndices.length)*8;  // assume 8 bytes per reference
        estBytes += (1 + hapIndices.length)*4; // 4 bytes per array to store length;
        for (int j=0; j<hapIndices.length; ++j) {
            if (j!=majorAllele) {
                estBytes += 4*(1 + hapIndices[j].length); // 4 bytes per array to store length;
            }
            else {
